...
public class Practice {
  ...
  private static int oddVerticesHelper(Vertex<Integer> starting, Set<Vertex<Integer>> visited){
    if(starting == null) return 0;
  ...
  public static List<Integer> sortedReachable(Vertex<Integer> starting) {
    List<Integer> sorted = sortedReachableHelper(starting, new HashSet<>(), new ArrayList<>());
  ...
  private static List<Integer> sortedReachableHelper(Vertex<Integer> starting, Set<Vertex<Integer>> visited, List<Integer> unsorted){
    if(starting == null) return unsorted;

    if(visited.contains(starting)) return unsorted;

    visited.add(starting);
    unsorted.add(starting.data);

    for(Vertex<Integer> neighbor : starting.neighbors){
      sortedReachableHelper(neighbor, visited, unsorted);
    }
  ...
  public static List<Integer> sortedReachable(Map<Integer, Set<Integer>> graph, int starting) {
    ...
    List<Integer> sorted = sortedReachableHelper(graph, starting, new HashSet<>(), new ArrayList<>());
  ...
  private static List<Integer> sortedReachableHelper(Map<Integer, Set<Integer>> graph, int starting, Set<Integer> visited, List<Integer> unsorted){
    if(graph.isEmpty())return unsorted;
    if(visited.contains(starting)) return unsorted;
    visited.add(starting);
    unsorted.add(starting);

    
        for(Integer neighbor : graph.get(starting) ){
            sortedReachableHelper(graph, neighbor, visited, unsorted);
        }
  ...
  public static boolean positivePathExists(Map<Integer, Set<Integer>> graph, int starting, int ending) {
    ...
    Set<Integer> set = positivePathExistsHelper(graph, starting, new HashSet<>());

    if(!set.contains(ending)) return false;

    return true;
  }

  private static Set<Integer> positivePathExistsHelper(Map<Integer, Set<Integer>> graph, int starting, Set<Integer> visited){
    if(visited.contains(starting)) return visited;
    if(starting < 0) return visited;

    visited.add(starting);

    for(Integer neighbor : graph.get(starting)){
      positivePathExistsHelper(graph, neighbor, visited);
    }
